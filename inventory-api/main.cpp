...
static std::string site_json(Row &r) {
  ...
  snprintf(buf, sizeof(buf),
           ...
           inventory_rules::available_capacity(to_int(r["TOTAL_CAP_MBPS"]), to_int(r["ALLOC_CAP_MBPS"])));
  return buf;
}
...
static std::string handle_circuits(const HttpRequest &req, int *status) {
  ...
  for (size_t i = 0; i < rows.size(); i++) {
    ...
    snprintf(buf, sizeof(buf),
             ...
             inventory_rules::available_capacity(to_int(r["CAP_MBPS"]), to_int(r["ALLOC_MBPS"])));
    if (i) out += ",";
    out += buf;
  }
  out += "]}";
...
static std::string handle_capacity(const HttpRequest &req, int *status) {
  ...
  std::string out = "{\"rule\":\"AVAIL_CAP_MBPS = TOTAL_CAP_MBPS - ALLOC_CAP_MBPS - MAINT_BUFFER_MBPS\",";
  char n[32];
  ...
  for (size_t i = 0; i < g_locations.size(); i++) {
    ...
    snprintf(buf, sizeof(buf),
             ...
             "\"TOTAL_CAP_MBPS\":%d,\"ALLOC_CAP_MBPS\":%d,\"MAINT_BUFFER_MBPS\":%d,"
             "\"AVAIL_CAP_MBPS\":%d,\"UTILIZATION_PCT\":%g,\"CAN_SUPPORT\":%s}",
             ...
             inventory_rules::utilization_pct(l.total_cap_mbps, l.alloc_cap_mbps, l.maint_buffer_mbps),
